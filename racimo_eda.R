...
df_food <- read.csv("D:/My Files/A-FreelanceWork/Racimo/raw_data/FAMILY INCOME AND EXPENDITURE SURVEY (2015) VOLUME 2 - FOOD EXPENDITURE - raw data.csv", header = T)
df_hh <- read.csv("D:/My Files/A-FreelanceWork/Racimo/raw_data/FAMILY INCOME AND EXPENDITURE SURVEY (2015) VOLUME 2 - HOUSEHOLD DETAILS AND HOUSING CHARACTERISTICS - raw data.csv", header = T)
df_income_r <- read.csv("D:/My Files/A-FreelanceWork/Racimo/raw_data/FAMILY INCOME AND EXPENDITURE SURVEY (2015) VOLUME 2 - INCOME AND OTHER RECEIPTS - raw data.csv", header = T)
df_nonfood <- read.csv("D:/My Files/A-FreelanceWork/Racimo/raw_data/FAMILY INCOME AND EXPENDITURE SURVEY (2015) VOLUME 2 - NONFOOD EXPENDITURE - raw data.csv", header = T)
df_totals <- read.csv("D:/My Files/A-FreelanceWork/Racimo/raw_data/FAMILY INCOME AND EXPENDITURE SURVEY (2015) VOLUME 2 - TOTALS OF INCOME AND EXPENDITURE - raw data.csv", header = T)
food_map <- read.csv("D:/My Files/A-FreelanceWork/Racimo/racimo/mapping_food_expend.csv", header=T) 
...
df_food <- left_join(df_food, select(df_totals, toinc, tfoodoutside, ttotex, concat), by="concat")
df_food[,c("wtoinc", "wtfoodoutside", "wttotex")] <- 0
df_food$tfoodoutside <- as.numeric(df_food$tfoodoutside)
df_food[,c("wtoinc", "wtfoodoutside", "wttotex")] <- df_food[,"rfact"]*df_food[,c("toinc", "tfoodoutside", "ttotex")]

#RECODING INCOME CLASSES
df_food <- df_food %>% 
            mutate(toinc_class = case_when(toinc < 40000 ~ "Under 40,000", 
...
df_food$broad_rgns <- recode(df_food$w_regn, 
...
#FOOD EXPENDITURE ALLOCATION - IN-HOME
df_fexpend <- df_food %>%
                select(wtrice:wtfothernonalcohol, wtfoodoutside, broad_rgns, toinc_class, members_class) %>% 
                gather(key=food_expend, value=texpend, wtrice:wtfoodoutside)
              

#EXPORT AGGREGATE DATA OF TOTAL FOOD EXPENSES
write.csv(df_fexpend, "D:/My Files/A-FreelanceWork/Racimo/racimo/food_expend.csv")
